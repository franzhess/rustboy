use super::{tests::machine_with_program, CpuState};
use crate::Machine;
...
fn values(machine: &Machine) -> [u8; 8] {
    [
        machine.cpu.registers.b,
        machine.cpu.registers.c,
        machine.cpu.registers.d,
        machine.cpu.registers.e,
        machine.cpu.registers.h,
        machine.cpu.registers.l,
        machine.mmu.read_byte(MEMORY),
        machine.cpu.registers.a,
    ]
}

fn seed(machine: &mut Machine, values: [u8; 8], flags: u8) {
    machine.cpu.registers.b = values[0];
    machine.cpu.registers.c = values[1];
    machine.cpu.registers.d = values[2];
    machine.cpu.registers.e = values[3];
    machine.cpu.registers.h = values[4];
    machine.cpu.registers.l = values[5];
    machine.mmu.write_byte(MEMORY, values[6]);
    machine
        .cpu
        .registers
        .set_af(u16::from(values[7]) << 8 | u16::from(flags));
    machine.cpu.registers.pc = 0x100;
}

fn step(machine: &mut Machine, opcode: u8, cycles: usize, pc: u16) {
    let result = machine.step();
    assert_eq!(
        (result.opcode, result.cycles, machine.cpu.registers.pc),
...
fn register_load_matrix_preserves_sources_flags_and_original_hl_address() {
    for opcode in 0x40..=0x7Fu8 {
        ...
        for flags in (0..=0xF0).step_by(16) {
            let mut machine = machine(&[opcode], flags);
            let mut expected = values(&machine);
            ...
            assert_eq!(values(&machine), expected, "opcode={opcode:02X}");
            assert_eq!(machine.cpu.registers.flags.bits(), flags);
            assert_eq!(machine.cpu.registers.sp, 0xC100);
        }
    }
}
        ...
        for operation in [4, 5, 6] {
            ...
            for value in 0..=u8::MAX {
                for flags in (0..=0xF0).step_by(16) {
                    ...
                    assert_eq!(
                        (values(&machine), machine.cpu.registers.flags.bits()),
    ...
    for operation in 0..8 {
        for source in 0..9 {
            ...
            for a in [0, 0x0F, 0x7F, 0x80, 0xFF] {
                for flags in (0..=0xF0).step_by(16) {
                    let mut machine = machine(&[opcode, 0x81], flags);
...
fn pairs(machine: &Machine) -> [u16; 4] {
    [
...
fn set_pair(machine: &mut Machine, pair: usize, value: u16) {
    match pair {
        0 => machine.cpu.registers.set_bc(value),
        1 => machine.cpu.registers.set_de(value),
        2 => machine.cpu.registers.set_hl(value),
        _ => machine.cpu.registers.sp = value,
    }
}
    ...
    for pair in 0..4 {
        for base in [0x01, 0x03, 0x0B, 0x09] {
            ...
            for value in [0, 0x07FF, 0x0800, 0x0FFF, 0x8000, 0xFFFF] {
                for flags in (0..=0xF0).step_by(16) {
                    let mut machine = machine(&[opcode, 0x34, 0x12], flags);
                    set_pair(&mut machine, pair, value);
                    let mut expected = pairs(&machine);
                    ...
                    assert_eq!(
                        (pairs(&machine), machine.cpu.registers.flags.bits()),
                        (expected, expected_flags),
                        "opcode={opcode:02X}"
                    );
                    assert_eq!(machine.cpu.registers.a, 0x9A);
                }
            }
        }
    }
    ...
    for (store, load, pair, delta) in [
        ...
        for opcode in [store, load] {
            for address in [0xC000, 0xCFFF, 0xFFFF] {
                let mut machine = machine(&[opcode], 0xF0);
                set_pair(&mut machine, pair, address);
                machine.mmu.write_byte(address, 0x35);
                let mut expected_pairs = pairs(&machine);
                if pair == 2 {
                    expected_pairs[2] = (i32::from(address) + delta) as u16;
                }
                step(&mut machine, opcode, 8, 0x101);
                assert_eq!(pairs(&machine), expected_pairs);
                assert_eq!(
                    machine.cpu.registers.a,
                    if opcode == store { 0x9A } else { 0x35 }
                );
                assert_eq!(
                    machine.mmu.read_byte(address),
                    if opcode == store { 0x9A } else { 0x35 }
                );
                assert_eq!(machine.cpu.registers.flags.bits(), 0xF0);
            }
        }
    }
    let mut machine = machine(&[0x3A], 0xF0);
    machine.cpu.registers.set_hl(0);
    step(&mut machine, 0x3A, 8, 0x101);
    assert_eq!(
        (machine.cpu.registers.get_hl(), machine.cpu.registers.a),
        (0xFFFF, 0)
    );
}
...
fn conditional_control_flow_checks_each_condition_and_preserves_stack_on_untaken_paths() {
    for (jr, jp, call, ret, mask, required) in [
        ...
        for flags in (0..=0xF0).step_by(16) {
            ...
            {
                let mut machine = machine(&[opcode, 0x34, 0x12], flags);
                machine.mmu.write_word(0xC0FE, 0xBEEF);
                machine.mmu.write_word(0xC100, 0x5678);
                let pc = if taken {
                    if opcode == ret {
                        0x5678
                    } else {
                ...
                assert_eq!(machine.mmu.read_word(0xC100), 0x5678);
                assert_eq!(machine.cpu.registers.flags.bits(), flags);
            }
            for offset in [0, 0x7F, 0x80, 0xFF] {
                let mut machine = machine(&[jr, offset], flags);
                let pc = if taken {
                    (0x102i32 + i32::from(offset as i8)) as u16
                } else {
                    0x102
                };
                step(&mut machine, jr, if taken { 12 } else { 8 }, pc);
                assert_eq!(machine.cpu.registers.flags.bits(), flags);
                assert_eq!(machine.cpu.registers.sp, 0xC100);
            }
        }
...
fn stack_pairs_and_restart_vectors_preserve_return_addresses_and_mask_af() {
    for (index, push, pop) in [
        ...
        let mut machine = machine(&[push, pop], 0xF0);
        let original = [
            machine.cpu.registers.get_bc(),
            machine.cpu.registers.get_de(),
            machine.cpu.registers.get_hl(),
        ...
        step(&mut machine, push, 16, 0x101);
        assert_eq!(machine.cpu.registers.sp, 0xC0FE);
        assert_eq!(machine.mmu.read_word(0xC0FE), original[index]);
        machine.mmu.write_word(0xC0FE, 0x12FF);
        step(&mut machine, pop, 12, 0x102);
    ...
    for (opcode, vector) in [
        ...
        let mut machine = machine(&[opcode], 0xF0);
        step(&mut machine, opcode, 16, vector);
        assert_eq!(machine.cpu.registers.sp, 0xC0FE);
        assert_eq!(machine.mmu.read_word(0xC0FE), 0x101);
        assert_eq!(machine.cpu.registers.flags.bits(), 0xF0);
    }
}
...
fn special_loads_use_high_memory_absolute_addresses_and_little_endian_words() {
    for (opcode, program, cycles) in [
        ...
        let mut machine = machine(&program, 0xF0);
        machine.cpu.registers.c = 0x80;
        machine.mmu.write_byte(0xFF80, 0x35);
        step(&mut machine, opcode, cycles, 0x100 + program.len() as u16);
        let store = matches!(opcode, 0xE0 | 0xE2 | 0xEA);
        assert_eq!(
            (machine.cpu.registers.a, machine.mmu.read_byte(0xFF80)),
            if store { (0x9A, 0x9A) } else { (0x35, 0x35) }
        );
        assert_eq!(machine.cpu.registers.flags.bits(), 0xF0);
    }
    let mut machine = machine(&[0x08, 0xFF, 0xC1, 0xF9], 0xF0);
    step(&mut machine, 0x08, 20, 0x103);
    assert_eq!(
        (machine.mmu.read_byte(0xC1FF), machine.mmu.read_byte(0xC200)),
        (0x00, 0xC1)
    );
    step(&mut machine, 0xF9, 8, 0x104);
    assert_eq!(
        (machine.cpu.registers.sp, machine.cpu.registers.flags.bits()),
        (MEMORY, 0xF0)
    );
}
...
fn accumulator_miscellaneous_opcodes_select_the_correct_operation() {
    for (opcode, a, flags, expected_a, expected_flags) in [
        ...
        step(&mut machine, opcode, 4, 0x101);
        assert_eq!(
            (values(&machine), machine.cpu.registers.flags.bits()),
            (expected, expected_flags)
        );
...
fn unconditional_control_flow_and_stop_keep_their_special_semantics() {
    let mut machine = machine(&[0xCD, 0x34, 0x12], 0xF0);
    step(&mut machine, 0xCD, 24, 0x1234);
    assert_eq!(
        (machine.cpu.registers.sp, machine.mmu.read_word(0xC0FE)),
        (0xC0FE, 0x103)
    );
    // Execute RET from WRAM, where the fixture can supply a handler byte.
    machine.cpu.registers.pc = 0xC200;
    machine.mmu.write_byte(0xC200, 0xC9);
    step(&mut machine, 0xC9, 16, 0x103);
    assert_eq!(machine.cpu.registers.sp, 0xC100);
    machine.cpu.registers.pc = 0xC200;
    machine.mmu.write_byte(0xC200, 0xE9);
    step(&mut machine, 0xE9, 4, MEMORY);
    machine.cpu.registers.pc = 0xC200;
    machine.mmu.write_byte(0xC200, 0x18);
    machine.mmu.write_byte(0xC201, 0xFE);
    step(&mut machine, 0x18, 12, 0xC200);
    machine.mmu.write_byte(0xC200, 0x10);
    machine.mmu.write_byte(0xC201, 0);
    step(&mut machine, 0x10, 4, 0xC202);
    assert_eq!(machine.cpu.state, CpuState::Stopped);
    assert_eq!(machine.cpu.registers.flags.bits(), 0xF0);
}
...
fn halt_is_not_a_memory_load_and_alu_memory_sources_are_read_only() {
    for opcode in [0x76, 0x86, 0x8E, 0x96, 0x9E, 0xA6, 0xAE, 0xB6, 0xBE] {
        let mut machine = machine(&[opcode], 0);
        machine.cpu.registers.set_hl(0xFF04);
        machine.mmu.do_ticks(256);
        step(
            &mut machine,
            opcode,
            if opcode == 0x76 { 4 } else { 8 },
            0x101,
        );
        assert_eq!(machine.mmu.read_byte(0xFF04), 1, "opcode={opcode:02X}");
        assert_eq!(
            machine.cpu.state,
            if opcode == 0x76 {
                CpuState::Halted
