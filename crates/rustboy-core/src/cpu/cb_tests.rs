...
fn operands(machine: &Machine) -> [u8; 8] {
    [
        ...
        machine.mmu.read_byte(MEMORY_OPERAND),
...
fn every_cb_opcode_has_the_expected_effect_for_all_values_and_flags() {
    for opcode in 0..=u8::MAX {
        let mut machine = machine_with_program(&[0xCB, opcode]);
        ...
        for value in 0..=u8::MAX {
            for flags in (0..=0xF0u8).step_by(0x10) {
                ...
                machine.cpu.registers.b = expected[0];
                machine.cpu.registers.c = expected[1];
                machine.cpu.registers.d = expected[2];
                machine.cpu.registers.e = expected[3];
                machine.cpu.registers.h = expected[4];
                machine.cpu.registers.l = expected[5];
                machine.mmu.write_byte(MEMORY_OPERAND, expected[6]);
                machine
                    .cpu
                    .registers
                ...
                assert_eq!(
                    (
                        operands(&machine),
                        machine.cpu.registers.flags.bits(),
...
fn bit_indirect_hl_does_not_write_back_to_a_side_effecting_register() {
    for opcode in (0x46..=0x7Eu8).step_by(8) {
        ...
        assert_eq!(machine.mmu.read_byte(0xFF04), 1);
        ...
        assert_eq!(machine.mmu.read_byte(0xFF04), 1, "CB {opcode:02X}");
    }
}
...
fn modifying_indirect_hl_writes_back_even_when_the_value_does_not_change() {
    ...
    for opcode in opcodes {
        ...
        assert_eq!(machine.mmu.read_byte(0xFF04), 0, "CB {opcode:02X}");
    }
}
